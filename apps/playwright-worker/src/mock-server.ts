...
export function startMockSite(port = 18791) {
  const server = createServer(async (req, res) => {
    ...
    if (pathname === '/me/applications') { const body = Buffer.from('<!doctype html><html><body><a href="/me/applications">My Applications</a></body></html>'); res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'content-length': body.length }); res.end(body); return; }
