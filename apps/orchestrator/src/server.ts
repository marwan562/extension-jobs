...
function authorize(req: IncomingMessage, sessions: Sessions, store: Store, scope: ClientScope | undefined, toolToken?: string, composioToolToken?: string): void { const suppliedOpenClaw = req.headers['x-openclaw-tool-token']; if (toolToken && typeof suppliedOpenClaw === 'string' && secretMatches(suppliedOpenClaw, toolToken)) { if (!store.authorizeClient('openclaw', suppliedOpenClaw, scope)) throw new HttpError(403, `Client lacks ${scope ?? 'required'} scope`); return; } const suppliedComposio = req.headers['x-composio-tool-token']; if (composioToolToken && typeof suppliedComposio === 'string' && secretMatches(suppliedComposio, composioToolToken)) { if (!store.authorizeClient('composio-host', suppliedComposio, scope)) throw new HttpError(403, `Client lacks ${scope ?? 'required'} scope`); return; } const auth = req.headers.authorization; if (!auth?.startsWith('Bearer ') || !sessions.valid(auth.slice(7))) throw new HttpError(401, 'Client authentication required'); }
