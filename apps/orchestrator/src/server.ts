...
import { clientScopes, type ClientScope } from '../../../packages/shared-contracts/src/index.ts';
...
export interface BridgeOptions { allowedOrigin: string; pairingCode: string; sessionTtlMs?: number; toolToken?: string; composioToolToken?: string; openclawScopes?: ClientScope[]; composioScopes?: ClientScope[] }
...
export function createBridge(service: OrchestratorService, options: BridgeOptions) {
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      authorize(req, sessions, service.store, requiredScope(url.pathname), options.toolToken, options.composioToolToken);
      ...
      if (req.method === 'GET' && url.pathname === '/v1/dashboard') return json(res, 200, { campaigns: service.store.listCampaigns(), profiles: service.store.listProfiles().map(publicProfile), timeline: service.store.timeline().map((event) => ({ ...event, detail: sanitizeAuditDetail(event.detail) })), agentSettings: service.settings, emergencyStop: service.emergencyStop.active });
...
const defaultAgentScopes: ClientScope[] = ['profile:read', 'jobs:search', 'applications:prepare', 'applications:fill', 'applications:review', 'campaigns:read', 'campaigns:manage', 'audit:read', 'admin:emergency-stop'];
function requiredScope(pathname: string): ClientScope | readonly ClientScope[] | undefined { const action = pathname.match(/^\/v1\/wuzzuf\/tools\/([A-Z_]+)$/)?.[1]; if (action) { if (['WUZZUF_SEARCH_JOBS', 'WUZZUF_GET_JOB_DETAILS', 'WUZZUF_SCORE_JOB', 'WUZZUF_GET_AUTH_STATUS', 'WUZZUF_OPEN_LOGIN', 'WUZZUF_CREATE_CONNECTION', 'WUZZUF_VERIFY_CONNECTION', 'WUZZUF_DISCONNECT'].includes(action)) return 'jobs:search'; if (action === 'WUZZUF_PREPARE_APPLICATION') return 'applications:prepare'; if (['WUZZUF_FILL_APPLICATION', 'WUZZUF_CANCEL_APPLICATION'].includes(action)) return 'applications:fill'; if (['WUZZUF_GET_APPLICATION_REVIEW', 'WUZZUF_GET_APPLICATION_STATUS', 'WUZZUF_REQUEST_SUBMISSION_APPROVAL'].includes(action)) return 'applications:review'; if (action === 'WUZZUF_SUBMIT_APPLICATION') return 'applications:submit'; } if (pathname.startsWith('/v1/profiles/') || pathname === '/v1/profiles/import' || pathname === '/v1/agent-settings') return 'profile:write'; if (pathname.startsWith('/v1/campaigns')) return 'campaigns:manage'; if (pathname === '/v1/emergency-stop' || pathname === '/v1/emergency-stop/reset') return 'admin:emergency-stop'; if (pathname === '/v1/dashboard') return ['profile:read', 'campaigns:read', 'audit:read']; if (pathname === '/v1/chat' || pathname === '/v1/models' || pathname === '/v1/answers/prepare') return 'profile:read'; return undefined; }
