...
export class OrchestratorService {
  ...
  async runCampaign(campaign: JobCampaign): Promise<{ correlationId: string; jobs: Job[]; applications: Array<{ id: string; job: Job; answers: FieldAnswer[] }> }> {
    ...
    try {
      ...
      const profile = this.store.getProfile(campaign.profileId) ?? this.store.registeredCandidateProfile(campaign.profileId); if (!profile) throw new Error('Profile not found');
  ...
  profileContext(profileId?: string): string { const profile = this.candidateProfile(profileId); if (!profile) return 'No approved candidate profile has been imported.'; const facts = profile.facts.filter((f) => f.path !== 'source.rawText').map((f) => `- [${f.id}] ${f.path}: ${String(f.value)} (${f.kind})`).join('\n'); return `Candidate profile: ${profile.name}\n${facts}`; }
