...
export interface HostConfig { apiKey: string; userId: string; hostToken: string; host: '127.0.0.1'; port: number; sessionFile: string; orchestratorUrl: string; pairingCode: string; toolkits: string[] }

export function loadConfig(): HostConfig {
  const apiKey = required('COMPOSIO_API_KEY'); const hostToken = required('COMPOSIO_HOST_TOKEN', 32); const pairingCode = required('WUZZUF_ORCHESTRATOR_PAIRING_CODE');
  ...
  return { apiKey, hostToken, pairingCode, host: '127.0.0.1', port, userId: process.env.COMPOSIO_USER_ID ?? 'local-user', sessionFile: resolve(process.env.COMPOSIO_SESSION_FILE ?? '.data/composio-session.json'), orchestratorUrl: url.origin, toolkits };
}
