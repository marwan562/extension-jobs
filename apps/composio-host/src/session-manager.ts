...
export class ComposioSessionManager {
  ...
  async initialize(): Promise<void> {
    const toolkit = this.wuzzufToolkit(); const sessionId = this.readSessionId();
    if (sessionId) { try { this.session = await this.composio.sessions.use(sessionId, { customToolkits: toolkit ? [toolkit] : [] }); this.initializedAt = new Date().toISOString(); return; } catch { /* create a replacement only during lifecycle initialization */ } }
    this.session = await this.composio.sessions.create(this.config.userId, { toolkits: this.config.toolkits.includes('linkedin') ? ['linkedin'] : [], manageConnections: true, experimental: { customToolkits: toolkit ? [toolkit] : [] } }); this.initializedAt = new Date().toISOString(); this.persistSessionId(this.session.sessionId);
  }
