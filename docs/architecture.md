...
SQLite uses WAL, unique job fingerprints, submission reservations, application records, approval-token hashes, resume BLOBs, locks, and append-only audit rows. Resume data and the connected Chrome profile remain local. The extension service worker owns its short-lived bearer token; the page content script never receives it. OpenClaw uses a separate environment-only backend token. Composio custom tools execute in-process and pair over loopback; no Wuzzuf credentials are sent to Composio.
