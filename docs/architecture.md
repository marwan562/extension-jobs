...
SQLite uses WAL, unique job fingerprints, submission reservations, application records, server-side approval requests, operation idempotency records, Wuzzuf connection state, resume BLOBs, locks, and append-only audit rows. Resume data and the connected Chrome profile remain local. The extension service worker owns its short-lived bearer token; the page content script never receives it. Only that paired extension session can decide an approval request. OpenClaw uses a separate configured backend token which cannot grant approval. Composio custom tools execute in-process and pair over loopback; no Wuzzuf credentials are sent to Composio.
